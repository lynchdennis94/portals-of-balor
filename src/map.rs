...
/// Makes a map of solid boundaries and 400 randomly placed walls
...
pub fn new_map_rooms_and_corridors() -> (Vec<super::Rect>, Vec<TileType>) {
    let mut map = vec![TileType::Wall; (MAP_HEIGHT * MAP_WIDTH) as usize];

    let mut rooms: Vec<super::Rect> = Vec::new();
...
fn apply_room_to_map(room: &super::Rect, map: &mut [TileType]) {
